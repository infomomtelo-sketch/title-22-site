...
No URL is recorded here on purpose. Ask whoever holds the video where it lives
...
player, says the file is not on the site yet, points at the guided tour at
`/videos/tour/` — which does play — and at the app, and leaves the transcript
fully usable. That path is tested; so is the one where the file loads.
